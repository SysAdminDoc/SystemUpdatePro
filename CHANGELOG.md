...
- Added maintenance-window and Intune-policy gating, deterministic cluster reboot leases, reversible High performance power-plan handling, interactive reboot approval, stage progress, and a tracked dry-run persistent-mutation contract.
- Added Azure Monitor/Sentinel-friendly webhook event data, an atomic Prometheus textfile exporter, parseable XML event-log payloads, and color-aware console output with legacy-console fallback.
